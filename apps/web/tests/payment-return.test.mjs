...
const sample = {
  id: "synthetic-order_123",
  userId: "buyer",
  plan: { name: "合成测试权益", level: 2 },
  amountCents: 1200,
...
test(
  "payment return: authenticated status, bounded polling, login continuation and readonly channel URLs",
  { timeout: 60000 },
  async (t) => {
    ...
    async function fixture({
      ...
      await page.route("**/*", async (route) => {
        ...
        if (path === `/api/orders/${sample.id}`) {
          ...
          const response =
            model.orderStatus === 200
              ? {
                  json: {
                    ...sample,
                    plan: { ...sample.plan, name: detailName(model.details) },
                    state: model.state,
                    paymentUrl: "https://gateway.invalid/synthetic-checkout",
                    reviewReason:
                      model.state === "paid_review"
                        ? "付款通知超过兑换有效期"
                        : undefined,
                  },
                }
          ...
          if (model.deferDetails) {
            model.pendingDetails.push(() => route.fulfill(response));
            return;
          }
        ...
        if (path === "/api/payment-channels")
          return route.fulfill({
            json: { channels: [{ id: "synthetic-channel", name: "合成渠道" }] },
          });
        if (path === "/api/orders" && method === "POST") {
          ...
          return route.fulfill({
            ...
            json: {
              ...sample,
              channelId,
              state: channelId === "balance" ? "paid" : "pending",
              paymentUrl:
                channelId === "balance"
                  ? undefined
                  : "https://gateway.invalid/synthetic-checkout",
            },
          });
        }
        if (path === "/api/orders" || path === "/api/admin/orders")
          return route.fulfill({ json: { orders: [] } });
        if (path === "/api/admin/payment-channels")
          return route.fulfill({
            json: {
              channels: [
                {
                  ...
                  configured: true,
                  notifyUrl:
                    "https://shop.example/api/payments/epay/synthetic-channel/notify",
                  returnUrlTemplate: "https://shop.example/?order={orderId}",
                },
              ],
            },
          });
    ...
    async function detailSettled(page, model, count) {
      ...
      const panel = page.getByRole("region", { name: "兑换返回结果" });
      ...
      await expect(
        panel.getByRole("button", { name: "刷新兑换状态", exact: true }),
      ).toBeEnabled();
    }
    try {
      await t.test(
        "component reused without a key isolates a new order and refreshes each paid order",
        async () => {
      ...
      `,
          ...
          await page.route("**/*", (route) => {
            const url = new URL(route.request().url());
            if (url.origin !== base) return route.abort();
            if (url.pathname === "/component-harness")
              return route.fulfill({
                contentType: "text/html",
                body: '<div id="fixture"></div>',
              });
            if (url.pathname === "/api/orders/order-A")
              return route.fulfill({
                json: {
                  ...sample,
                  id: "order-A",
            ...
            if (url.pathname === "/api/orders/order-B") {
              secondRequest = route;
              return;
            }
            return route.abort();
          ...
          try {
            ...
            await expect(
              page.getByRole("region", { name: "兑换返回结果" }),
            ).toContainText("第一个权益");
            assert.deepEqual(await page.evaluate(() => window.paymentSignals), [
              "order-A",
            ]);
            await page.evaluate(() => window.renderPaymentOrder("order-B"));
            const panel = page.getByRole("region", { name: "兑换返回结果" });
            await expect(panel).toContainText("兑换编号：order-B");
            await expect(panel).not.toContainText("第一个权益");
            await expect(panel).not.toContainText("兑换成功，权益已生效");
            await expect.poll(() => !!secondRequest).toBe(true);
            await secondRequest.fulfill({
              json: {
                ...sample,
                id: "order-B",
                state: "paid",
            ...
            await expect(panel).toContainText("第二个权益");
            assert.deepEqual(await page.evaluate(() => window.paymentSignals), [
              "order-A",
              "order-B",
            ]);
      ...
      await t.test(
        "forged success ignored; 13 reads maximum; manual refresh confirms only server paid",
        async () => {
          const { page, model, requests, errors } = await fixture();
          try {
            await page.goto(
              `/?order=${sample.id}&trade_status=TRADE_SUCCESS&state=paid&sign=untrusted`,
            );
            const panel = page.getByRole("region", { name: "兑换返回结果" });
            await expect(panel).toContainText("等待兑换结果");
            await expect(panel).not.toContainText("兑换成功，权益已生效");
            ...
            await panel
              .getByRole("button", { name: "刷新兑换状态", exact: true })
              .click();
            await expect(panel).toContainText("兑换成功，权益已生效");
      ...
      await t.test(
        "delayed response does not overlap reads; next poll starts after that response settles",
        async () => {
          const { page, model, requests, errors } = await fixture();
          try {
      ...
      await t.test(
        "login keeps order destination; does not query orders before authentication",
        async () => {
          ...
          try {
            ...
            await expect(page).toHaveURL(
              new RegExp(`\\?order=${sample.id}#orders$`),
            );
            await expect(
              page.getByRole("region", { name: "兑换返回结果" }),
            ).toContainText("兑换成功，权益已生效");
            assert.deepEqual(
      ...
      for (const state of ["paid_review", "expired"])
        await t.test(
          `${state} stays distinct and stops automatic polling`,
          async () => {
      ...
      await t.test(
        "foreign/missing order displays only authorized API error",
        async () => {
          const { page, model } = await fixture({ orderStatus: 404 });
          try {
            await page.goto(`/?order=${sample.id}&state=paid`);
            const panel = page.getByRole("region", { name: "兑换返回结果" });
            await expect(panel).toContainText("兑换记录不存在");
            await expect(panel).not.toContainText("合成测试权益");
            await page.clock.runFor(70000);
            assert.equal(model.details, 1);
          } finally {
            await page.close();
      ...
      await t.test(
        "stalled request times out and manual refresh remains available",
        async () => {
          ...
          try {
            ...
            const panel = page.getByRole("region", { name: "兑换返回结果" });
            await expect(panel).toContainText("兑换记录查询超时，请手动刷新");
            model.hang = false;
            await panel
              .getByRole("button", { name: "刷新兑换状态", exact: true })
              .click();
      ...
      for (const channelId of ["synthetic-channel", "balance"])
        await t.test(
          `created ${channelId} order preserves correct payment feedback`,
          async () => {
            const { page, model, requests } = await fixture();
            try {
              ...
              await page
                ...
                .click();
              await page.getByLabel("兑换方式").selectOption(channelId);
              await page
                .getByLabel("我理解新权益将覆盖旧的剩余时间和流量")
                .check();
              await page
                .getByRole("button", { name: "确认兑换", exact: true })
                .click();
              const dialog = page.getByRole("dialog");
              if (channelId === "balance") {
                await expect(dialog).toContainText("兑换成功，权益已生效");
                assert.equal(model.details, 0);
              } else {
                await expect(dialog).toContainText("等待兑换结果");
                await expect(
                  dialog.getByRole("link", { name: "前往支付", exact: true }),
                ).toBeVisible();
                model.state = "paid";
                await dialog
                  .getByRole("button", { name: "刷新兑换状态", exact: true })
      ...
      await t.test(
        "malformed or repeated order selectors never construct an API path",
        async () => {
          const { page, requests } = await fixture();
          try {
            for (const query of [
              "?order=..%2Fadmin%2Forders",
              "?order=a&order=b",
              "?order=https%3A%2F%2Fevil.invalid",
            ]) {
      ...
      await t.test(
        "admin can inspect generated callback and return template without editing them",
        async () => {
          ...
          try {
            ...
            const returning =
              page.getByLabel("返回地址 / 同步跳转入口（模板）");
            await expect(notify).toHaveValue(
              "https://shop.example/api/payments/epay/synthetic-channel/notify",
            );
            await expect(returning).toHaveValue(
              "https://shop.example/?order={orderId}",
            );
