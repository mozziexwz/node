...
bbr_conf=/etc/sysctl.d/99-msboost-bbr.conf
[ ! -L "$bbr_conf" ]
if [ -e "$bbr_conf" ]; then
  [ -f "$bbr_conf" ]
  [ "$(stat -c %u -- "$bbr_conf")" = 0 ]
fi
cat > "$work/99-msboost-bbr.conf" <<'MSBOOST_BBR_CONF'
...
MSBOOST_BBR_CONF
chmod 0644 "$work/99-msboost-bbr.conf"
if [ ! -f "$bbr_conf" ] || ! cmp -s "$work/99-msboost-bbr.conf" "$bbr_conf"; then
  install -o root -g root -m 0644 "$work/99-msboost-bbr.conf" "$bbr_conf"
fi
