...
[`v0.3.3`](https://github.com/mozziexwz/node/releases/tag/v0.3.3) 已正式发布，[标签 CI run 36163149523](https://github.com/mozziexwz/node/actions/runs/36163149523) 全绿，包含 5 分钟与 30 分钟耐久门槛。9 个公开 Release 资产的大小和 SHA-256 均已核对，部署归档的 282 个文件、24 个目录与固定标签逐成员一致；GHCR 双架构镜像均完成匿名实际拉取。现有控制面及独立 Agent 不会自动升级。新装控制面建议使用 Debian 12/13，旧站升级须安排维护窗口并核对备份；完整结果和边界见[正式发布后验收记录](acceptance-vps-20260926.md)。

- 免费「部署 MSBOOST」只保留全新安装，不再向用户提供“安全升级/修复”。每次部署重新生成受管节点配置与随机端口，**不执行系统 DD**。部署和自备中转入口在提交前提示仅支持 Debian，并先在目标 VPS 检查 Debian 11 或更新版本；不符合条件时应在更改 sysctl、下载或安装前拒绝。控制面自身仍只支持 Debian 12/13；面板「DD 系统」目标仍固定 Debian 12。
- 权益管理删除特殊的「体验权益每位用户仅可兑换一次」选项，统一改为各权益的「每人限兑次数（0 不限兑）」。管理员可对指定权益重置所有用户的限兑计数；这不删除历史订单，也不撤销仍有效的权益。枫叶兑换码可设置单码允许使用次数，同一用户对同一码至多兑换一次。
- 下架权益仍向当前有效持有人显示并允许其续兑；非持有人及已过期持有人不能新购或兑现旧待付订单。已确认 v2 身份或仍有关联线路的节点拒绝普通注册令牌轮换，后台不再把这类节点展示为可直接“部署/重装”，避免误操作令健康管理连接离线。
- 针对控制面已全新安装、旧 Relay 私有身份仍留在节点的情况，新增需双重确认的 `--fresh-reset --acknowledge-relay-restart` 重注册流程：先核验受管状态与文件归属，将旧状态留在 root 私有备份，再用新单次令牌注册。已有 v2 身份时普通重跑会在变更前拒绝，不会因粘贴新令牌自动覆盖旧身份；旧转发会中断，务必先确认可维护。新装 Relay 除本机服务启动外，还须在 30 秒内取得持久注册身份、首个受认证的控制同步及有效控制 epoch，且未进入恢复核对，安装器才报告完成；仍须在后台核对在线心跳和实际转发。控制面已标记 `recovery_required` 的节点不能靠全新重置自动解除，须走受信恢复。节点注册令牌与持久状态不要手动公开、删除或复用。
- 修复管理菜单 13–15 所用恢复流程中 Docker socket `-H` 参数被重复传入的问题，保留异常备份核销、受保护 Relay 恢复与 TLS 维护入口，不直接移除安全功能。

候选源码阶段已完成：5 台授权临时 VPS（覆盖 Debian 11/12/13）的部署前 Debian 版本只读检查；3 台旧 Relay 节点使用候选 Agent 显式全新重置后均在现有控制面显示在线且取得有效 `controlEpoch`，其中两台重置前分别有 1 条和 3 条旧规则。最终安全补丁加入后，Go 全套测试、`go vet`、前端构建、隔离 HTTP 9/9、浏览器 41/41 与安装器相关 Shell 回归均通过。这些属于发布前候选证据；正式制品及发布后实机结果另列于验收记录，不能用候选测试替代。

发布后，v0.3.2 控制面升级至 v0.3.3，原有 1 个管理员账号与 3 台 Relay 保留、Relay 保持在线，另有 1 台新版 Executor 在线。正式 v0.3.3 控制面执行 `msboost repair` 成功：版本、管理员凭据、主密钥、数据库密码及业务记录数量均保持不变，3 台 Relay 与 1 台 Executor 修复后恢复在线；这不是整站卸载/恢复验收。Debian 11/12/13 三台客户 VPS 分别通过全新部署、新配置、BBR/FQ 和公网 TCP 核验。Debian 11 自备中转通过公网 TCP，并经“预览 2 项受管资源 → 摘要确认清理”验证：中转单元移除，原直连服务与配置保留。Ubuntu 22.04 与 Debian 10 在隔离容器中被实际前置脚本拒绝，未进入主机变更。**本轮未实测**游戏协议、真实支付和邮件投递、arm64 真机运行、v0.3.3 DD、完整整站卸载/恢复及容量上限；这些不能写作已通过。可使用以下固定入口安装本版：

```sh
curl -fsSL --proto '=https' --tlsv1.2 https://raw.githubusercontent.com/mozziexwz/node/v0.3.3/install.sh -o /root/msboost-install.sh
bash /root/msboost-install.sh
```

## v0.3.2 · 真实 VPS 排障与平台兼容

源码版本号本身不代表正式发布。v0.3.2 须等标签 CI、双架构镜像及 Release 资产全部完成后才可按下列入口安装。
...
```sh
curl -fsSL --proto '=https' --tlsv1.2 https://raw.githubusercontent.com/mozziexwz/node/v0.3.2/install.sh -o /root/msboost-install.sh
bash /root/msboost-install.sh
```
