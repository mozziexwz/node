...
本节描述源码候选改动，**不代表标签、Release 资产、双架构镜像或真实 VPS 验收已经完成**。安装前须核对正式 Release、校验清单和 CI 结果；现有控制面及独立 Agent 不会自动升级。本轮推荐在全新 Debian 12/13 控制面上安装并独立核验，不把旧版本业务数据迁移作为验收前提。
...
候选源码阶段已完成：5 台授权临时 VPS（覆盖 Debian 11/12/13）的部署前 Debian 版本只读检查；3 台旧 Relay 节点使用候选 Agent 显式全新重置后均在现有控制面显示在线且取得有效 `controlEpoch`，其中两台重置前分别有 1 条和 3 条旧规则。最终安全补丁加入后，Go 全套测试、`go vet`、前端构建、隔离 HTTP 9/9、浏览器 41/41 与安装器相关 Shell 回归均通过。这些结果证明相应前置检查、受管重注册与本机回归，**不等于**客户 VPS 真实全新部署、非 Debian 拒绝路径、既有业务不中断、游戏连接或公开 Release 制品已经验收。

发布前仍需完成：真实用户端全新部署及拒绝路径验证，并对本标签 CI（含 30 分钟 Relay 耐久）、双架构镜像、Release 资产与 SHA256 清单独立核验。历史 v0.3.2 VPS 记录不能代替本轮结果。待正式制品发布并核对后，可使用以下固定入口：
