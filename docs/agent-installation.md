...
### 本地工作树：显式 keep_last 迁移（尚未发布）

配套新版本发布后，中转安装可显式追加 `--offline-policy keep_last`；对已运行的服务还需逐次传 `--acknowledge-relay-restart`，明确承认 Agent/GOST 升级会中断原连接。默认 `lease` 继续兼容旧链，不因安装器版本号自动启用 v2。旧发布二进制缺少该能力时，在停止原服务前拒绝。
