...
安装先保存旧文件/服务状态，再更新和重启。服务启动失败时尝试恢复原程序、私有环境、systemd unit、启用状态和之前的运行状态，并保留备份。Relay 节点若已有 v2 身份，普通安装会在变更前拒绝；保留身份升级使用下述显式原地升级，舍弃旧身份则只能使用下述全新重置。不要用已消费节点注册令牌进行一次新的注册。
...
4. **不要将已有 keep_last 节点的普通重跑当成升级。** 安装器发现旧 v2 身份时，会在停止服务前拒绝：本地持久 `managementToken`/`agentId` 会优先于新粘贴的注册令牌。若要保留旧身份和规则，请在该节点的维护窗口执行显式 `--upgrade-in-place --acknowledge-relay-restart`，不输入或传入新注册令牌。安装器要求原控制面地址、受管单元、已完成认证同步的私有 v2 状态及现有身份一致；仅替换程序/单元，保留状态和身份，升级后的环境不再保存已使用的注册令牌。Agent/GOST 重启仍会断开现有 TCP 连接，绝非无损热升级。若旧控制面已丢失或节点处于恢复冻结，应走[受信中转恢复](relay-recovery.md)，不能靠升级换管理身份。只有旧状态可丢弃且旧转发全部允许中断时，才执行下节的双重确认全新重置。不要手动删除所有权标记、状态或令牌。
...
已有 `keep_last` 节点的固定版本原地升级示例（先逐跳安排维护，确认旧连接可中断）：
...
这里不使用 `--token-file`，也不会提示粘贴令牌。安装器不会清空 `relay-v2-state.json`、规则或流量日志；完成后仍须核对原节点 ID、后台在线、逐跳新版本能力和实际转发。免费自备中转由客户 VPS 的配置任务部署，不是此 Agent；旧实例须重新执行受控配置流程后才有新版协议筛查。
...
节点删除与 VPS 清理分开。只有后台对原 ID 完成无关联业务、停止证明等检查并成功删除后，后台才会显示该 ID 对应的 `deploy/uninstall-agent.sh` 固定版本本机清理命令；未显示时不要手工拼接。管理员须在原 VPS root 终端独立核对 Release 资产、版本、节点 ID、控制面地址及将删除的受管状态和私有备份后再执行。该脚本不作用于网站容器或其他 VPS；后台删除本身不卸载远端软件。
...
对已经运行的 Relay，任何需要重启 Agent/GOST 的安装或策略切换都可能中断原连接。未持有 v2 私有身份的活动 lease 服务迁移到 keep_last 时必须逐次传 `--offline-policy keep_last --acknowledge-relay-restart`，明确承认这次维护风险；缺少确认时安装器在停止旧服务前拒绝。已有 v2 身份普通重跑会在变更前拒绝，保留身份只接受 `--upgrade-in-place --acknowledge-relay-restart` 且不接受新注册令牌。建议优先准备全新 Debian 12 节点并完整验收，再安排旧节点维护迁移，而不是把版本升级描述为无感切换。
