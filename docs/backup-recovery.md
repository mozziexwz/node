...
适用版本：v0.2.0 起。所有步骤都不会重装客户 VPS，也不会恢复客户 VPS 的磁盘。
...
1. 在设置开启维护模式，结束运行/排队任务，暂停本站转发，并等待已下发租约失效（通常至少 45 秒）。
...
5. 重新登录，核对文章附件与配置，重新注册/关联 Agent；按新关联检查线路并重新生成必要的用户规则，再解除维护。旧任务不会自动重放。
