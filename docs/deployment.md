...
默认目标为 **Debian 12，amd64 或 arm64**。安装器下载指定 GitHub Release 的部署包并校验 SHA256，默认使用预构建镜像 `ghcr.io/mozziexwz/node:v0.1.2`，不要求在 VPS 编译 Go 或网页。
...
在承载网站的 VPS 上以 root 运行。推荐先将真实域名解析到该 VPS，开放 TCP 80、443，并确保服务器能够访问 Docker、GitHub 和 GHCR。安装目录固定为 `/opt/msboost`，Compose 项目名为 `msboost`。

```sh
apt-get update
apt-get install -y ca-certificates curl tar
curl --fail --location --proto '=https' --tlsv1.2 \
  https://raw.githubusercontent.com/mozziexwz/node/v0.1.2/install.sh \
...
将示例域名、邮箱替换为自己的值。执行 `bash /root/msboost-install.sh` 不带动作参数时显示交互菜单。首次安装默认版本为 `v0.1.2`；也可传入明确的 `--version vX.Y.Z`。
...
管理员环境变量只用于首次引导。数据库已有管理员时，修改 `ADMIN_PASSWORD` 不会重置密码；应在用户管理中重置。
...
```sh
curl -fsSL --proto '=https' --tlsv1.2 https://raw.githubusercontent.com/mozziexwz/node/v0.1.2/install.sh -o /root/msboost-install-v0.1.2.sh && \
  bash /root/msboost-install-v0.1.2.sh upgrade --version v0.1.2 --recover-incomplete
```
...
```sh
msboost upgrade --version v0.1.2 --build
```
...
| 字段 | 用途 |
|---|---|
| `MSBOOST_VERSION` | 固定发布版本，默认 `v0.1.2` |
| `MSBOOST_IMAGE` | 默认从 `ghcr.io/mozziexwz/node:v0.1.2` 获取，取得后固定 digest；归档方式记录独立本地标签 |
...
网络为 `172.30.86.0/24`，Caddy 固定 `172.30.86.2`，服务只信任 `TRUSTED_PROXY_CIDRS=172.30.86.2/32`。网络冲突时必须同时调整三者，不得信任整个公网；增加 CDN/其他代理时重新核验可信链。
...
1. HTTPS、登录、权限隔离、CSRF、真实 SMTP 投递；注册验证、免费工具验证、购买套餐验证三个策略分别验证。
2. Executor 领取任务、SSH 指纹、安装/修复与配置获取；DD 单独确认目标磁盘和备份，并核验重启后的系统。
3. 节点/隧道实际转发与计量、断网租约撤销、重启和到期清理、客户端认证。
4. 真实支付商户的金额、签名、重复/晚到回调及异常款；同步回跳不是到账凭证。
5. 加密平台备份、独立主密钥和隔离恢复；数据库快照不包含客户 VPS 整盘或浏览器保存的免费配置。

在线恢复要求维护模式、无运行任务及本站租约失效等条件，并先产生回滚备份。如果旧快照会回退后续财务、卡密、支付去重、权益或流量，服务拒绝直接覆盖，需保留当前数据离线对账。恢复后退出会话并撤销 Agent 令牌，需要重新接入。完整范围见 [安全模型与限制](security-and-limits.md)。
