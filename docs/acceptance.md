...
## 2026-09-15 至 2026-09-22 补充2.3 / Relay v2 验证

本节记录整改期间的增量验证，不覆盖上文各历史版本当时的结论。最终源码已随 v0.2.4 发布；生产控制面和客户节点不会自动升级。

- 最新本机 Go 全包测试及静态检查、TypeScript/Vite 构建、HTTP 9 项和浏览器 35 项通过；新增 root 维护入口、恢复/终态历史、混合安装回滚和保护私有凭据的反例测试。
- 用户授权临时 Debian 12 上，固定校验真实 GOST 3.3.0、生产 Run 和三拓扑（单跳TCP、双跳TCP、三跳TLS）各5/30分钟全部通过。每个原TCP仅拨号1次、重连0次；30分钟各1799次回显，满足本轮耐久标准。管理端为分阶段受控故障服务器，不是实际面板停机30分钟；独立真实后端连续停站30分钟的证据见[分层验收](relay-v2-verification.md)。
...
- 本轮没有重启生产控制面、真实商户扣款或向真实客户邮箱发信。v0.2.4 双架构镜像和公开资产已发布并独立核验，授权临时 VPS 已完成一键安装生命周期测试；原有客户节点未自动更新。

完整 T01–T26 分层映射及未测项见 [Relay v2 验收证据](relay-v2-verification.md)。30 分钟耐久和手动/定时灾备与原TCP的联合验收现已通过；真实QQ邮件渲染等其他未测场景完成前，不能宣称“全部生产功能验收通过”。
