...
(
  cd "$out"
  sha256sum "msboost-deploy-${version}.tar.gz" install-agent.sh msboost-agent-linux-amd64 msboost-agent-linux-arm64 msboost-image-linux-amd64.tar.gz msboost-image-linux-arm64.tar.gz > SHA256SUMS
)
