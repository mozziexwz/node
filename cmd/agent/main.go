...
func main() {
	capability := flag.String("capability", "executor", "executor or relay (separate tokens and machines recommended)")
	...
	case "relay":
		err = relayruntime.Run(ctx, relayruntime.Config{ServerURL: *server, EnrollmentToken: os.Getenv("MSBOOST_RELAY_ENROLLMENT_TOKEN"), StateDir: *state, GostBinary: *gost})
	default:
		log.Fatal("unsupported capability: use executor or relay")
	}
