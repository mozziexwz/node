...
awk '/raw.githubusercontent.com\/mozziexwz\/node\/v1.0.0\/agent.sh/ {sub("/v1.0.0/agent.sh", "/v0.2.4/agent.sh")} {print}' "$TEST_REPO/apps/web/src/admin.tsx" > "$version_fixture/apps/web/src/admin.tsx"
...
sed '0,/"version": "1.0.0"/s//"version": "0.2.4"/' "$TEST_REPO/apps/web/package-lock.json" > "$version_fixture/apps/web/package-lock.json"
...
tar -czf "$TEST_WORK/release/msboost-deploy-v1.0.0.tar.gz" -C "$TEST_WORK/fixture" deploy install.sh
...
cp -- "$TEST_WORK/release/msboost-deploy-v1.0.0.tar.gz" "$TEST_WORK/release/msboost-deploy-v0.2.3.tar.gz"
(cd "$TEST_WORK/release" && sha256sum msboost-deploy-v1.0.0.tar.gz msboost-deploy-v0.2.3.tar.gz > SHA256SUMS)
...
curl() {
  ...
  [[ -n $output ]] || return 22
  case "$url" in https://github.com/mozziexwz/node/releases/download/v1.0.0/*|https://github.com/mozziexwz/node/releases/download/v0.2.3/*) ;; *) return 22 ;; esac
...
grep -qx 'v1.0.0' "$BOOTSTRAP_TEST_TRACE" || fail 'release version not forwarded'
...
rm -- "$BOOTSTRAP_TEST_TRACE"
(bootstrap_main upgrade --version v1.0.0 --recover-incomplete)
...
rm -- "$BOOTSTRAP_TEST_TRACE"
MOCK_LATEST_BODY='{"url":"https://api.github.com/repos/mozziexwz/node/releases/1","id":1,"tag_name":"v1.0.0","target_commitish":"main","assets":[],"body":"release note with escaped \"tag_name\": \"v9.9.9\""}'
(bootstrap_main upgrade)
grep -qx 'upgrade' "$BOOTSTRAP_TEST_TRACE" || fail 'compact GitHub release response did not resolve upgrade'
grep -qx 'v1.0.0' "$BOOTSTRAP_TEST_TRACE" || fail 'compact release tag parsed incorrectly'
rm -- "$BOOTSTRAP_TEST_TRACE"
MOCK_LATEST_BODY='{"url":"https://api.github.com/repos/mozziexwz/node/releases/1","tag_name":"v1.0.0/../../bad"}'
...
printf '%064d  msboost-deploy-v1.0.0.tar.gz\n' 0 > "$TEST_WORK/release/SHA256SUMS"
...
MOCK_FETCH_FAIL=0
if (bootstrap_main install --version 'v1.0.0/../../bad'); then fail 'unsafe release version accepted'; fi
