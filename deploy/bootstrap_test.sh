...
tar -czf "$TEST_WORK/release/msboost-deploy-v0.2.1.tar.gz" -C "$TEST_WORK/fixture" deploy install.sh
(cd "$TEST_WORK/release" && sha256sum msboost-deploy-v0.2.1.tar.gz > SHA256SUMS)
...
curl() {
  ...
  [[ $url == https://github.com/mozziexwz/node/releases/download/v0.2.1/* && -n $output ]] || return 22
...
grep -qx 'v0.2.1' "$BOOTSTRAP_TEST_TRACE" || fail 'release version not forwarded'
...
rm -- "$BOOTSTRAP_TEST_TRACE"
(bootstrap_main upgrade --version v0.2.1 --recover-incomplete)
...
rm -- "$BOOTSTRAP_TEST_TRACE"
MOCK_LATEST_BODY='{"url":"https://api.github.com/repos/mozziexwz/node/releases/1","id":1,"tag_name":"v0.2.1","target_commitish":"main","assets":[],"body":"release note with escaped \"tag_name\": \"v9.9.9\""}'
(bootstrap_main upgrade)
grep -qx 'upgrade' "$BOOTSTRAP_TEST_TRACE" || fail 'compact GitHub release response did not resolve upgrade'
grep -qx 'v0.2.1' "$BOOTSTRAP_TEST_TRACE" || fail 'compact release tag parsed incorrectly'
rm -- "$BOOTSTRAP_TEST_TRACE"
MOCK_LATEST_BODY='{"url":"https://api.github.com/repos/mozziexwz/node/releases/1","tag_name":"v0.2.1/../../bad"}'
...
printf '%064d  msboost-deploy-v0.2.1.tar.gz\n' 0 > "$TEST_WORK/release/SHA256SUMS"
...
MOCK_FETCH_FAIL=0
if (bootstrap_main install --version 'v0.2.1/../../bad'); then fail 'unsafe release version accepted'; fi
